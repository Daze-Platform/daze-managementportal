...
// Windrose Restaurant menu items
const BASE_ITEMS: Array<
  Omit<ProductMixItem, "quantity" | "grossSales" | "netSales" | "avgPrice">
> = [
  // Snacks & Small Plates
  {
    id: "1",
...
// Price mapping for Windrose Restaurant items
...
export function getProductMixData(
  storeId: string,
  dateRange?: DateRange,
  ...
  const days = dateRange?.from
    ? Math.max(1, Math.floor((toTs - fromTs) / MS_DAY) + 1)
    : 7; // default 7 days if none selected

  const seed = hashString(
    `${storeId || "all"}|${fromTs}|${toTs}|${includeModifiers ? "1" : "0"}`,
  );
  ...
  return items.map((base, idx) => {
    ...
    const basePerDay =
      ((itemSeed % 7) + 3) * (idx % 4 === 0 ? 2 : idx % 3 === 0 ? 1.5 : 1.0);
    ...
    const price = PRICE_MAP[base.name] || 15; // Use actual Windrose Restaurant pricing
...
export function groupByCategory(items: ProductMixItem[]) {
  const map = new Map<
    string,
    { category: string; quantity: number; grossSales: number; netSales: number }
  >();
  items.forEach((it) => {
