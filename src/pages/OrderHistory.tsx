  ...
  const filteredOrders = React.useMemo(() => {
    const allOrdersData = [
      ...
      },
      {
        ...
        status: "Completed",
        items: [
          { id: "9", name: "Charbroiled Octopus", price: 32.5, quantity: 2 },
          {
            id: "10",
            name: "Artisan Bread & Olive Oil",
            price: 13.11,
            quantity: 2,
          },
        ],
    ...
    if (selectedStore !== "all") {
      console.log(
        `OrderHistory: Filtering orders for store ID "${selectedStore}"`,
      );
      result = result.filter((order) => order.storeId === selectedStore);
      console.log(
        `OrderHistory: Found ${result.length} orders for store "${selectedStore}"`,
      );
    } else {
      ...
      switch (selectedStatus) {
        ...
        case "refunded":
          result = result.filter(
            (o) =>
              (o as any).refundStatus && (o as any).refundStatus !== "none",
          );
          break;
      }
      console.log(
        `OrderHistory: Applied status filter "${selectedStatus}" → ${result.length} orders`,
      );
    }
            ...
            >
              ...
              >
                <Select value={selectedStore} onValueChange={setSelectedStore}>
                  <SelectTrigger
                    className={`border-gray-200 bg-white ${scrollDirection === "down" && !isAtTop ? "w-32 h-8 text-xs" : "w-full sm:w-48"}`}
                  >
