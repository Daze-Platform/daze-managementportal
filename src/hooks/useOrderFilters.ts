...
export const useOrderFilters = () => {
  const applyAdvancedFilters = (orders: Order[], advancedFilters: FilterState): Order[] => {
    let filtered = orders;
    ...
    if (advancedFilters.orderValue) {
      filtered = filtered.filter(order => {
        ...
        switch (advancedFilters.orderValue) {
          case 'under25': return amount < 25;
          case '25to50': return amount >= 25 && amount <= 50;
    ...
    if (advancedFilters.customerType.length > 0) {
      filtered = filtered.filter(order => {
        const customerTypes = ['New Customer', 'Returning Customer', 'VIP Customer', 'Corporate'];
        const orderCustomerType = customerTypes[parseInt(order.id) % customerTypes.length];
  ...
  const getFilteredOrders = (
    ...
    console.log('getFilteredOrders called with:', {
      ...
      allOrderStoreIds: orders.map(o => ({ id: o.id, storeId: o.storeId, storeName: o.storeName, isVisible: o.isVisible }))
    });
    
    // Filter by store - only if a specific store is selected
    if (selectedStore !== 'all') {
      console.log(`🔍 Filtering by store: "${selectedStore}"`);
      console.log(`📦 Orders to filter:`, filtered.map(o => ({ 
        id: o.id, 
        storeId: o.storeId, 
      ...
      filtered = filtered.filter(order => {
        ...
        const matchesByName = order.storeName && order.storeName.toLowerCase() === selectedStore.toLowerCase();
        const matches = matchesById || matchesByName;
        
        console.log(`🔸 Order ${order.id}: storeId="${order.storeId}", storeName="${order.storeName}", selectedStore="${selectedStore}", matches=${matches}`);
        
        return matches;
      });
      console.log(`📊 Store filtering result: ${beforeFilter} → ${filtered.length} orders for store "${selectedStore}"`);
    ...
    if (orderType !== 'all') {
      ...
      filtered = filtered.filter(order => 
        orderType === 'pickup' ? order.type === 'Pick Up' : order.type === 'Delivery'
      );
      console.log(`Type filtering (${orderType}): ${beforeTypeFilter} → ${filtered.length} orders`);
    }

    // Apply advanced filters
    const beforeAdvancedFilter = filtered.length;
    filtered = applyAdvancedFilters(filtered, advancedFilters);
    console.log(`Advanced filtering: ${beforeAdvancedFilter} → ${filtered.length} orders`);
    
    console.log('Final filtered result:', filtered.map(o => ({ 
      id: o.id, 
      storeId: o.storeId, 
      customer: o.customer,
  ...
  const getOrderTypeCount = (
    ...
    activeTab: string,
    selectedStore: string,
    type: 'pickup' | 'delivery' | 'new' | 'progress' | 'ready' | 'fulfillment' | 'fulfilled' | 'scheduled'
  ): number => {
    if (type === 'pickup' || type === 'delivery') {
      ...
      return filteredByStore.filter(order => 
        type === 'pickup' ? order.type === 'Pick Up' : order.type === 'Delivery'
      ).length;
    } else {
      ...
      console.log(`getOrderTypeCount for ${type} tab with store ${selectedStore}: ${filteredCount} orders`);
