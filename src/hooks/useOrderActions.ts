...
export const useOrderActions = (
  ...
  const handleOrderUpdate = (orderId: string, action: 'accept' | 'decline' | 'ready' | 'complete' | 'fulfill' | 'schedule' | 'activate', data?: any) => {
    console.log('Order update action:', action, 'for order:', orderId);
    
    // Find the order to check its type
    const currentOrder = orderData[activeTab as keyof typeof orderData]?.find(order => order.id === orderId);
    ...
    switch (action) {
      ...
      case 'complete':
        ...
        break;
      case 'fulfill':
        console.log('Moving order from fulfillment/ready to fulfilled:', orderId);
      ...
      case 'schedule':
        ...
        scheduleOrder(orderId, activeTab as keyof typeof orderData, scheduledTime);
