...
export const useOrderManagement = () => {
  ...
  const { handleOrderSelect, handleOrderUpdate, moveOrder, removeOrder, scheduleOrder } = useOrderActions(
    activeTab,
    selectedOrder,
    setSelectedOrder,
    setShowOrderDetails,
  ...
  useEffect(() => {
    ...
    if (orderStatus === 'active' && storeStatus === 'open' && orderData.new.length === 0) {
      console.log('useOrderManagement: User clicked "Start Taking Orders" - generating orders for ALL stores');
  ...
  }, [orderStatus, storeStatus, orderData.new.length, generateInitialOrders, setIsWaitingForOrders]);
  ...
  useEffect(() => {
    if (orderData.new.length > 0) {
      console.log('useOrderManagement: Orders exist, ensuring no waiting state for filtering');
      setIsWaitingForOrders(false);
    }
  ...
  const getOrderTypeCountWithParams = (type: 'pickup' | 'delivery' | 'new' | 'progress' | 'ready' | 'fulfillment' | 'fulfilled' | 'scheduled') => 
