...
describe('useFullScreenScroll', () => {
  ...
  it('removes the orientationchange listener with the same reference it was added with', () => {
    ...
    const orientationAdd = addCalls.find(([type]) => type === 'orientationchange');
    expect(orientationAdd).toBeDefined();

    unmount();

    const removeCalls = (window.removeEventListener as ReturnType<typeof vi.spyOn>).mock.calls;
    const orientationRemove = removeCalls.find(([type]) => type === 'orientationchange');
