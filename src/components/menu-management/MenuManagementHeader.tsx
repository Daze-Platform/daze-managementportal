...
export const MenuManagementHeader: React.FC<MenuManagementHeaderProps> = ({ selectedStore, onSelectStore }) => {
  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-6 min-h-[120px] sm:min-h-[140px]">
      <div className="min-w-0 flex-1">
        ...
        <p className="text-gray-600 text-sm sm:text-base leading-relaxed">Create and manage your restaurant menus</p>
      </div>
            ...
            >
              <div className="flex items-center space-x-2">
                <div className="text-left min-w-0 flex-1">
                  <div className="font-medium text-gray-900 text-sm hover:text-blue-700 transition-colors duration-200 truncate">{selectedStore.name}</div>
                  <div className="text-xs text-gray-500 truncate hidden sm:block">{selectedStore.location}</div>
                </div>
              </div>
              <ChevronDown className="w-4 h-4 text-gray-400 flex-shrink-0" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-64 sm:w-72 bg-white/95 backdrop-blur-lg border border-gray-200/50 shadow-xl rounded-lg p-2 z-50">
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center space-x-3 min-w-0 flex-1">
                    <div className="min-w-0 flex-1">
                      <div className="font-medium text-gray-900 text-sm hover:text-blue-700 transition-colors duration-200 truncate">{store.name}</div>
