...
const MODE_META: Record<Mode, { title: string; sub: string; warning?: string }> = {
  manual: {
    title: "Manual",
  ...
  auto: {
    ...
    warning: "Make sure couriers are signed up and toggled online before you flip this — otherwise orders fall through to the Available queue.",
  },
  hybrid: {
    title: "Hybrid",
...
export const DispatchModeCard = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (error || !data) {
        toast({ title: "Failed to load dispatch settings", description: error?.message ?? "Tenant not found", variant: "destructive" });
        setLoading(false);
        return;
      }
  ...
  const handleSave = async () => {
    ...
    if (readErr || !tRow) {
      toast({ title: "Could not save", description: readErr?.message ?? "Tenant not found", variant: "destructive" });
      setSaving(false);
      return;
    }
    const merged = { ...((tRow.settings ?? {}) as TenantSettings), dispatch_mode: mode };
    const { error } = await supabase.from("tenants").update({ settings: merged }).eq("id", tenantId);
    if (error) {
      toast({ title: "Save failed", description: error.message, variant: "destructive" });
    } else {
      toast({ title: "Dispatch mode updated", description: `${tenantName || "Tenant"} is now in ${MODE_META[mode].title} mode.`, variant: "success" });
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Controls how new orders get assigned to your couriers. You can flip this at any time — changes take effect on the next order.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <RadioGroup value={mode} onValueChange={(v) => setMode(v as Mode)} className="space-y-3">
          {(Object.keys(MODE_META) as Mode[]).map((key) => {
            ...
            return (
              ...
              >
                <RadioGroupItem id={`mode-${key}`} value={key} className="mt-1" />
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          {dirty && (
            <Button variant="outline" onClick={() => setMode(originalMode)} disabled={saving}>
              Cancel
            </Button>
          )}
