        ...
        <div className="flex items-center space-x-4">
          ...
          {item.modifierGroups.length > 0 && (
            <div className="flex items-center space-x-2">
              <Badge variant="outline" className="text-xs bg-blue-50 text-blue-600 border-blue-200 flex items-center space-x-1">
                <Settings className="w-3 h-3" />
                <span>{item.modifierGroups.length} Option Set{item.modifierGroups.length !== 1 ? 's' : ''}</span>
              </Badge>
            </div>
          )}
        </div>
