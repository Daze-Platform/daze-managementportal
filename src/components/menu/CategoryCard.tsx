...
export const CategoryCard: React.FC<CategoryCardProps> = ({
  ...
  return (
    <Card className="bg-white border border-gray-200/50 shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
      <CardHeader className={`border-b border-gray-100 ${isMobileOrTablet ? 'p-3 sm:p-4' : 'p-4'}`}>
        <div className={`flex ${isMobileOrTablet ? 'flex-col gap-3' : 'items-center justify-between gap-3'}`}>
          <div className="flex items-center flex-1 min-w-0">
            {isEditing ? (
              <div className="w-full space-y-2">
                <Label htmlFor={`category-name-${category.id}`} className="text-xs font-semibold text-gray-500">
                  Category Name
                </Label>
                <div className={`flex items-center gap-2 ${isMobileOrTablet ? 'flex-col' : ''}`}>
                  <Input
                    ...
                    className={`text-base font-semibold border-2 border-blue-300 focus:border-blue-500 ${
                      isMobileOrTablet ? 'h-12 text-lg' : 'h-11 sm:h-12 sm:text-lg'
                    }`}
                    autoFocus
                    placeholder="E.g. Appetizers"
                  />
                  <div className={`flex gap-2 ${isMobileOrTablet ? 'w-full' : 'flex-shrink-0'}`}>
                    <Button 
                      variant="ghost" 
                      size="sm"
                      onClick={handleSave}
                      className={`hover:bg-green-50 text-green-600 hover:text-green-700 ${
                        isMobileOrTablet ? 'flex-1 h-12 text-base' : 'p-2 h-11 w-11'
                      }`}
                    >
          ...
          <div className={`flex items-center gap-2 ${isMobileOrTablet ? 'w-full' : 'flex-shrink-0'}`}>
            <Button 
              size="sm" 
              variant="outline"
              onClick={handleAddItemClick}
              className={`border-blue-200 text-blue-600 hover:bg-blue-50 hover:border-blue-300 shadow-sm ${
                isMobileOrTablet ? 'flex-1 h-12 text-base font-semibold' : 'h-9 sm:h-10 text-xs sm:text-sm px-3 sm:px-4'
              }`}
            >
