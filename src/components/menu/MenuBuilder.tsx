...
export const MenuBuilder: React.FC<MenuBuilderProps> = ({ menu, onSave, onCancel, isNew }) => {
  ...
  const [availableModifierGroups] = useState<ModifierGroup[]>([
    ...
    },
    {
      ...
      options: [
        ...
        { id: '2', name: 'Ranch Dressing', price: 0.50 },
  ...
  const updateMenuItem = (categoryId: string, item: MenuItem) => {
    setCurrentMenu(prev => ({
      ...
      categories: prev.categories.map(cat => 
        cat.id === categoryId 
          ? { ...cat, items: cat.items.map(i => i.id === item.id ? item : i) }
          : cat
      )
    }));
    setShowItemForm(false);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className={`flex-1 ${isMobileOrTablet ? 'px-3 py-2' : 'px-4 sm:px-6 lg:px-8 py-3'}`}>
      ...
      <div className={`sticky bottom-0 z-20 bg-white border-t border-gray-200 shadow-lg ${
        ...
        <div className="max-w-7xl mx-auto">
          <div className={`flex ${isMobileOrTablet ? 'flex-col gap-3' : 'flex-row justify-end gap-3'}`}>
            <Button 
              variant="outline" 
              onClick={onCancel}
            ...
            <Button 
              ...
              className={`${
                isMobileOrTablet ? 'h-11 text-base font-semibold' : 'h-10 text-sm px-8 font-medium'
              }`}
            >
              Save Menu
            </Button>
