...
export const MenuHeader: React.FC<MenuHeaderProps> = ({
  ...
  return (
    <div className={`${isMobile ? 'space-y-4' : 'flex items-start gap-8'}`}>
      <div className={`${isMobile ? 'space-y-4' : 'flex items-start space-x-6'} flex-1 min-w-0`}>
        <Button 
          variant="ghost" 
          size="sm" 
          onClick={onCancel}
          className={`hover:bg-brand-blue/10 text-gray-600 hover:text-brand-blue ${
            isMobile ? 'w-full justify-start h-9 text-sm' : 'h-9 text-sm flex-shrink-0 mt-1'
          }`}
        >
        ...
        <div className={`${isMobile ? 'space-y-3' : 'flex-1 min-w-0 max-w-3xl space-y-3'}`}>
          {isNew && (
            <Label htmlFor="menu-name" className="text-sm font-medium text-gray-600">
              Menu Name
            </Label>
          )}
          <Input
