...
export const CategoryManager: React.FC<CategoryManagerProps> = ({
  ...
  return (
    <div className={`space-y-${isMobileOrTablet ? '4' : '5'}`}>
      {/* Compact Header */}
      <div className={`${isMobileOrTablet ? 'text-center space-y-3' : 'flex items-center justify-between'}`}>
        <div className={isMobileOrTablet ? 'space-y-1' : ''}>
          ...
          <p className={`text-gray-600 ${isMobileOrTablet ? 'text-sm' : 'text-sm'} mt-1`}>
            {isMobileOrTablet 
        ...
        <Button 
          ...
          className={`bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 text-white shadow-md hover:shadow-lg transition-all duration-200 ${
            isMobileOrTablet ? 'w-full h-10 text-sm font-semibold rounded-lg touch-manipulation' : 'h-9 text-sm'
          }`}
        >
          ...
          <CardContent className={`flex flex-col items-center justify-center ${
            ...
            <div className={`relative ${isMobileOrTablet ? 'mb-4' : 'mb-6'}`}>
              <div className={`${
                ...
                <ChefHat className={`${isMobileOrTablet ? 'w-8 h-8' : 'w-10 h-10'} text-blue-600`} />
            ...
            <h3 className={`${
              ...
              {isMobileOrTablet ? 'Create your first category' : 'Ready to build your menu?'}
            </h3>
            ...
            <Button 
              ...
              className={`bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 text-white shadow-md hover:shadow-lg transition-all duration-300 hover:scale-105 ${
                isMobileOrTablet ? 'w-full max-w-xs h-10 text-sm font-semibold rounded-lg touch-manipulation' : 'px-6 py-2 text-sm rounded-lg'
              }`}
            >
              <Plus className="w-4 h-4 mr-2" />
              {isMobileOrTablet ? 'Add First Category' : 'Create Your First Category'}
            </Button>
            
            {/* Compact Mobile Tips */}
            {isMobileOrTablet && (
              <div className="mt-6 space-y-2">
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Popular Categories</p>
                <div className="flex flex-wrap gap-1 justify-center">
                  {['🥗 Salads', '🍕 Pizza', '🍔 Burgers', '🍰 Desserts', '☕ Beverages'].map((category) => (
