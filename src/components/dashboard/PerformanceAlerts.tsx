...
export const PerformanceAlerts = ({ alerts }: PerformanceAlertsProps) => {
  const getUrgencyBadge = (urgency: string) => {
    switch (urgency) {
      ...
      case 'low':
        return <Badge className="bg-green-100 text-green-800 hover:bg-green-100 text-xs">🟢 Good</Badge>;
      default:
  ...
  const getVarianceIcon = (current: number, target: number, metric: string) => {
    const isGoodDirection = metric.includes('Rating') || metric.includes('Acceptance') || metric.includes('Availability');
    ...
    <Card className="shadow-sm h-full">
      <CardHeader className="pb-2 px-3 sm:px-6">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base sm:text-lg lg:text-xl">Platform Performance</CardTitle>
          <div className="flex items-center gap-1 sm:gap-2">
            <AlertTriangle className="w-4 h-4 sm:w-5 sm:h-5 text-orange-500" />
            <Badge className="bg-red-100 text-red-800 hover:bg-red-100 text-xs hidden sm:inline-flex">
              {alerts.filter(alert => alert.urgency === 'critical' || alert.urgency === 'high').length} Needs Attention
            </Badge>
            <Badge className="bg-red-100 text-red-800 hover:bg-red-100 text-xs sm:hidden">
              {alerts.filter(alert => alert.urgency === 'critical' || alert.urgency === 'high').length}
            </Badge>
          </div>
        </div>
      </CardHeader>
              ...
              <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1 mb-2 sm:mb-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 text-xs text-gray-500">
                    <span>Current: {formatValue(alert.currentValue, alert.metric)}</span>
                    <span className="hidden sm:inline">Target: {formatValue(alert.targetValue, alert.metric)}</span>
                  </div>
                  <div className="flex items-center gap-1 mt-1 text-xs text-gray-600">
                    {getVarianceIcon(alert.currentValue, alert.targetValue, alert.metric)}
                    <span>{getVariancePercentage(alert.currentValue, alert.targetValue)}% variance</span>
                  </div>
                </div>
              </div>
              <div className="flex justify-between sm:block sm:text-right flex-shrink-0">
                <div className="sm:hidden text-xs text-gray-500">Target: {formatValue(alert.targetValue, alert.metric)}</div>
                <div>
