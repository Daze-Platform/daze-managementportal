...
export const ActiveOrdersHeader = ({
  ...
  const getDesktopStatusMessage = () => {
    if (storeStatus === "closed") {
      return selectedStoreName === "All Venues"
        ? "All venues are closed"
        : `${selectedStoreName} is closed`;
    } else if (orderStatus === "paused") {
      return selectedStoreName === "All Venues"
        ? "Orders are paused for all venues"
        : `Orders are paused for ${selectedStoreName}`;
    } else {
      return selectedStoreName === "All Venues"
        ? "All venues are active"
        : `${selectedStoreName} is active`;
    }
  };
      ...
      <div className="block md:hidden">
        <div className="flex items-center justify-between space-x-2">
          <div className="flex items-center space-x-2 min-w-0 flex-1">
            ...
            <div
              className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${statusDisplay.statusDotColor}`}
            ></div>
            <span
              className={`text-xs font-medium truncate ${statusDisplay.statusColor}`}
            >
        ...
        <div className="flex items-center justify-between space-x-2">
          <div className="flex items-center space-x-2 min-w-0 flex-1">
            ...
            <div className="flex items-center space-x-1">
              <div
                className={`w-1.5 h-1.5 rounded-full ${statusDisplay.statusDotColor}`}
              ></div>
              <span
                className={`text-xs font-medium ${statusDisplay.statusColor}`}
              >
      ...
      <div className="hidden lg:flex lg:items-center lg:justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center space-x-2">
            <div
              className={`w-2 h-2 rounded-full ${statusDisplay.statusDotColor} ${statusDisplay.isActive ? "animate-pulse" : ""}`}
            ></div>
            <span
              className={`text-sm font-medium ${statusDisplay.statusColor}`}
            >
