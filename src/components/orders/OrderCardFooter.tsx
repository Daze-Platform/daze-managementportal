...
export const OrderCardFooter = ({ order }: OrderCardFooterProps) => {
  ...
  const getEstimatedTimeStyle = () => {
    if (order.estimatedTime === "Ready")
      return "text-green-700 bg-green-50 border-green-200";
    if (order.estimatedTime === "Completed")
  ...
  return (
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-3">
        <div
          className={`w-9 h-9 ${order.iconBg} rounded-lg flex items-center justify-center shadow-sm`}
        >
      ...
      {order.estimatedTime && (
        <div
          className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-full border ${getEstimatedTimeStyle()}`}
        >
