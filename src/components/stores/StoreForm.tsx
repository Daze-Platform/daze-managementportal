...
export const StoreForm = ({ isOpen, onClose, onSubmit, store }: StoreFormProps) => {
  ...
  const form = useForm<Store>({
    defaultValues: {
      ...
      activeOrders: 0,
      hours: days.map(day => ({ day, open: '09:00', close: '17:00', isOpen: false })),
      resortId: null,
    },
  });
  ...
  React.useEffect(() => {
    if (store) {
      ...
      form.reset({
        ...
        hours: store.hours || days.map(day => ({ day, open: '09:00', close: '17:00', isOpen: false })),
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent 
        className="w-full h-screen sm:w-[90vw] sm:h-[90vh] sm:max-w-[600px] sm:max-h-[800px] m-0 sm:m-auto p-0 rounded-none sm:rounded-lg border-0 sm:border [&>button]:hidden"
      >
        <div className="flex flex-col h-screen sm:h-full bg-background">
          ...
          <div className="flex-1 relative">
            ...
            >
              <div className="px-4 py-4 space-y-6">
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6" id="store-form">
                  <FormField
                    control={form.control}
                    name="name"
                    render={({ field }) => (
                        ...
                        <Select onValueChange={field.onChange} value={field.value || 'unassigned'}>
                          ...
                          <SelectContent>
                            <SelectItem value="unassigned">
                              <span className="text-gray-500">No resort assigned</span>
                            </SelectItem>
                            {resorts.map((resort) => (
                            ...
                            {colorOptions.map((color) => (
                              <SelectItem key={color.value} value={color.value}>
                                <div className="flex items-center space-x-3">
                                  <div className={`w-5 h-5 rounded ${color.value}`}></div>
          ...
          {/* Fixed Footer */}
          <div className="flex-shrink-0 border-t bg-background p-4 z-20" style={{ paddingBottom: 'calc(1rem + env(safe-area-inset-bottom))' }}>
