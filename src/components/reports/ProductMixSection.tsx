...
export const ProductMixSection: React.FC<ProductMixSectionProps> = ({ data }) => {
  ...
  const [channels, setChannels] = useState<Record<SalesChannel, boolean>>({
    "Dine-in": true,
    Takeout: true,
    Delivery: true,
  });
  ...
  const totalNet = useMemo(
    () => filtered.reduce((sum, it) => sum + it.netSales, 0),
    [filtered],
  );
  ...
  function exportCSV() {
    ...
    link.href = url;
    link.download = `product-mix-all.csv`;
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:items-center flex-wrap">
            ...
            <div className="flex flex-wrap gap-2">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" className="gap-2">
                    <ListFilter className="h-4 w-4" /> Channels
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent
                  align="end"
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" className="gap-2">
                    <Filter className="h-4 w-4" /> Dayparts
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent
                  align="end"
                ...
                >
                  ...
                  {(Object.keys(dayparts) as Daypart[]).map((d) => (
                    <DropdownMenuCheckboxItem
                      key={d}
                      checked={dayparts[d]}
                      onCheckedChange={() => toggleDaypart(d)}
                    >
                      {d}
                    </DropdownMenuCheckboxItem>
                  ))}
              ...
              <div className="flex items-center gap-2 px-2">
                <Checkbox
                  ...
                  onCheckedChange={(v) => setIncludeModifiers(v === true)}
                  className="h-4 w-4"
                />
                <label
                  htmlFor="modifiers"
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead
                  onClick={() => onHeaderClick("quantity")}
                  className="cursor-pointer select-none"
                >
                  Qty Sold
                </TableHead>
                <TableHead
                  onClick={() => onHeaderClick("grossSales")}
                  className="cursor-pointer select-none"
                >
                  Gross Sales
                </TableHead>
                <TableHead
                  onClick={() => onHeaderClick("netSales")}
                  className="cursor-pointer select-none"
                >
                  Net Sales
                </TableHead>
                <TableHead
                  onClick={() => onHeaderClick("avgPrice")}
                  className="cursor-pointer select-none"
                >
                  Avg Price
                </TableHead>
            ...
            <TableCaption>
              ...
              {groupBy === "item" ? "items" : "categories"}. Total Net Sales: $
              {totalNet.toFixed(2)}
            </TableCaption>
          </Table>
        </div>
